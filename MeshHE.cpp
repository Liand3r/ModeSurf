...
Mesh MeshHE::BillateralMeshDenoising( Mesh &m )
{
	...
	for(int j = 0; j < nb_iter; j++){
		m_denoised.ComputeNormals();
		double sigmaS, sigmaC;
		...
		for(glm::uint i=0; i < m.NbVertices();i++){
			...
			for(glm::uint k = 0; k<neighbors.size(); k++){
				double d = glm::distance(*(neighbors[k]->m_position),v);
				if(d < sigmaC){
					sigmaC = d;
				}
			}
			...
			offset /= (double)off_set_dist.size();
			...
			float normalizer = 0;	
			for(glm::uint k = 0; k<neighbors.size(); k++){
				vec3 voisin = *(neighbors[k]->m_position);
				double h = glm::dot(v-voisin,n);
				double t = glm::distance(voisin,v);
				double wc = std::exp(-0.5*t*t/(sigmaC*sigmaC));	
				double ws = std::exp(-0.5*h*h/(sigmaS*sigmaS));
				sum = sum + wc*ws*h;
